...
def clean_text_for_json(text: str) -> str:
    """Clean text to prevent Unicode encoding issues in JSON"""
    if not text:
        return ""
    
    # Convert to ASCII with replacement characters to avoid any Unicode issues
    return text.encode('ascii', errors='replace').decode('ascii')

def decode_uni_names(s):
    def repl(m):
        code = int(m.group(1), 16)
        # skip control chars if you like:
        if code < 32:  # control chars
            return ''  # or '\uFFFD'
        try:
            return chr(code)
        except ValueError:
            return ''  # skip invalid codes
    return re.sub(r'/uni([0-9A-Fa-f]{2,8})', repl, s)
...
class TopicModelingStep(ABC):
    ...
    def prepare_data(self, pdf_folder: str, output_dir: str, max_workers: int = 4) -> str:
        ...
        print(f"Prepared data_remaining.jsonl with {len(pdf_texts)} documents")
    ...
    def prepare_data_for_topicgpt(self, pdf_texts: List[Dict[str, str]]) -> str:
        ...
        for i, pdf_data in enumerate(pdf_texts):
            ...
            cleaned_text = clean_text_for_json(truncated_text)
            cleaned_filename = clean_text_for_json(pdf_data["filename"])
