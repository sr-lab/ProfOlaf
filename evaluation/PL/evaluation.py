...
def parse_rater_data(data):
    ...
    for item in data:
        if item[0] == "title":
            continue
        filename = item[0].replace(".pdf", "").replace(" ", "_").replace(":", "_").replace("/", "_").replace("\\", "_").replace("*", "_").replace("?", "_").replace("\"", "_").replace("<", "_").replace(">", "_").replace(".", "_")
        #print(filename)
        response_parsed = item[3].split(",")
        parsed_data[filename] = response_parsed
...
rater_data = read_csv_file('evaluation_agreement_pl.csv')
